...
async def resolve_all_issues(
    ...
    logger.info(
        f"Resolving issues with Agent {AGENT_CLASS}, model {llm_config.model}, max iterations {max_iterations}."
    )
...
def main():
    parser = argparse.ArgumentParser(description="Resolve all issues from Github.")
    parser.add_argument(
        "--repo",
        type=str,
        required=True,
