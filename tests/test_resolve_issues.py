...
async def test_process_issue_with_list_history(mock_output_dir, mock_prompt_template):
    ...
    test_cases = [
        {
            ...
            "expected_histories": [{"content": "", "type": "NullObservation"}]
        },
        {
            "name": "history_with_get_events",
            "history": MagicMock(get_events=lambda: [NullObservation(content="test")]),
            "expected_histories": [{"content": "test", "type": "NullObservation"}]
        }
    ]
    ...
    # Mock return values
    mock_create_runtime.return_value = MagicMock(connect=AsyncMock())
    # Simulate the bug where history is a list instead of an object with get_events()
    mock_run_controller.return_value = MagicMock(
        history=[NullObservation(content="")],  # This is the bug - history is a list
        metrics=MagicMock(get=MagicMock(return_value={"test_result": "passed"})),
        last_error=None,
    )
    mock_complete_runtime.return_value = {"git_patch": "test patch"}
    ...
    with patch(
        ...
        # The function should raise an AttributeError because history is a list
        with pytest.raises(AttributeError, match="'list' object has no attribute 'get_events'"):
            await process_issue(
                issue,
                base_commit,
                max_iterations,
