...
def update_pull_request(repo_name: str, pr_number: int) -> None:
    ...
    # Get associated issue numbers
    issue_numbers = get_associated_issue_numbers(pr)
    ...
    print(f"Associated issue numbers: {issue_numbers}")

def get_associated_issue_numbers(pr) -> List[int]:
    """Get issue numbers associated with the PR"""
    body = pr.body
    issue_numbers = [int(s[1:]) for s in body.split() if s.startswith('#') and s[1:].isdigit()]
