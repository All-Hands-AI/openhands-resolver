...
async def complete_runtime(
    ...
    while n_retries < 5:
        ...
        if isinstance(obs, CmdOutputObservation):
            if obs.exit_code == 0:
                new_git_patch = obs.content.strip()
                break
            else:
    ...
    # Check if there's an existing git patch
    existing_patch_file = os.path.join(workspace_base, 'existing_patch.diff')
    if os.path.exists(existing_patch_file):
        with open(existing_patch_file, 'r') as f:
            existing_git_patch = f.read()
        git_patch = existing_git_patch + '\n' + new_git_patch if new_git_patch else existing_git_patch
    else:
        git_patch = new_git_patch

    # Save the updated git patch
    with open(existing_patch_file, 'w') as f:
        f.write(git_patch)
...
async def process_issue(
    ...
    if os.path.exists(workspace_base):
        logger.info(f"Workspace for issue {issue.number} already exists. Resuming from existing workspace.")
    else:
        # copy the repo to the workspace
        shutil.copytree(os.path.join(output_dir, "repo"), workspace_base)
        logger.info(f"Created new workspace for issue {issue.number}.")
    ...
    config = AppConfig(
        ...
        # Use existing workspace if it exists, otherwise create a new one
        workspace_base=workspace_base,
        workspace_mount_path=workspace_base,
    )
    
    # If workspace already exists, update the config to use it
    if os.path.exists(workspace_base):
